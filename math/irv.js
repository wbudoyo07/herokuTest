const Irv = {
    ...
    readBallots: (ballotsText) => {
        const ballots = ballotsText.split('\n');
    ...
    validateInput: (candidates, ballots, incompleteBallots, threshold) => {
    ...
    countFirstVotes: (candidatesCount, ballots) => {
        return Irv.countNVotes(1, candidatesCount, ballots);
    },

    countNVotes: (n, candidatesCount, ballots) => {
    ...
    calculateRoundWinners: (firstVotes) => {
        var maxVotes = -1;
        var roundWinners = [];
    ...
    calculateRoundLosers: (firstVotes, ballotsCount) => {
    ...
    calculateRoundLosersOfCandidates: (candidates, firstVotes, ballotsCount) => {
        var minVotes = ballotsCount + 1;
        var roundLosers = [];
        for (var i = 0; i < candidates.length; i++) {
    ...
    removeLoserCandidate: (candidateNames, roundLoser) => {
        candidateNames.splice(roundLoser, 1);
        return candidateNames;
    },

    removeLoserFromBallots: (ballots, roundLoser) => {
        for (var i = 0; i < ballots.length; i++) {
    ...
    candidateIndexToName: (candidateNames, candidatesIndices) => {
    ...
    calculateWinner: (candidateNames, ballots, tiebreakerSecondary, threshold) => {
        var round = 0;
